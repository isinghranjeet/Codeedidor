...
import { 
  FaCode, FaQuestionCircle, FaTerminal, FaRobot, FaHistory, 
  FaChartLine, FaSun, FaMoon, FaCopy, FaVolumeUp, FaRedo, 
  FaSave, FaCog, FaPlay, FaLightbulb, FaInfoCircle, FaShare, 
  FaDownload, FaUpload, FaUsers, FaTrophy, FaMagic, FaUserFriends, 
  FaSyncAlt 
} from "react-icons/fa";
...
const DEFAULT_QUESTIONS = {
  ...
  Medium: [
    {
      title: "Basic Example",
      description: "Write a program that demonstrates basic features.",
      solution: "// Implement your solution here",
      explanation: "This demonstrates basic features.",
      testCases: [{ input: "", output: "Example output" }]
    }
  ],
  Hard: [
    {
      title: "Advanced Example",
      description: "Write a program that demonstrates advanced features.",
      solution: "// Implement your solution here",
      explanation: "This demonstrates advanced features.",
      testCases: [{ input: "", output: "Advanced output" }]
    }
  ]
};
...
const QUESTIONS = {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Sum: " + (num1 + num2));
        ...
        testCases: [{ input: "5 10", output: "Sum: 15" }]
      }
    ],
    Medium: [
      {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Factorial: " + factorial);
        ...
        testCases: [{ input: "5", output: "Factorial: 120" }]
      }
    ],
    Hard: [
      {
...
// Initialize questions for all languages
LANGUAGES.forEach(lang => {
  if (!QUESTIONS[lang.value]) {
    QUESTIONS[lang.value] = JSON.parse(JSON.stringify(DEFAULT_QUESTIONS));
    QUESTIONS[lang.value].Easy[0].description = `Write a ${lang.name} program to print 'Hello, World!'.`;
  }
});
...
const analyzeCodeQuality = (code) => {
  ...
  return { level, feedback, complexity, functions };
};
...
export default function CodingPage() {
  ...
  // Handlers
  const handleRunCode = useCallback(async (compileOnly = false) => {
    if (!code.trim()) {
      toast.warning("Please write some code before running!", { icon: "✏️" });
      return;
    }
    ...
    try {
      ...
      toast.error("Error executing code!", { icon: "❗" });
      console.error("Execution error:", error);
    } finally {
      setIsLoading(false);
      setTimeout(() => setProgress(0), 1000);
    }
  }, [code, language, userStats]);

  const requestAiCodeReview = useCallback(async () => {
    if (!code.trim()) {
      toast.warning("Please write some code before requesting a review!", { icon: "✏️" });
      return;
    }

    try {
      setIsLoading(true);
      toast.info("AI assistant is reviewing your code...", { icon: <FaRobot /> });
      ...
      toast.success("AI code review completed!", { icon: "🤖" });
    } catch (error) {
      toast.error("AI review failed", { icon: "❌" });
    } finally {
      setIsLoading(false);
    }
  }, [code, difficulty, codeLevel, language]);

  const formatCode = useCallback(() => {
    if (!code.trim()) {
      toast.warning("No code to format!", { icon: "✏️" });
      return;
    }
    ...
    try {
      ...
      setCode(formatted);
      toast.success("Code formatted!", { icon: "✨" });
    } catch (error) {
      toast.error("Formatting failed", { icon: "❌" });
    }
  }, [code]);

  const syncWithCollaborators = useCallback(() => {
    if (!showCollaboration) {
      toast.warning("Collaboration mode is not active!", { icon: "👥" });
      return;
    }

    if (collaboratorCode) {
      setCode(prev => prev + "\n" + collaboratorCode);
      toast.success("Incorporated collaborator changes!", { icon: "🔄" });
      setCollaboratorCode("");
    } else {
      toast.info("No new changes from collaborators", { icon: "👥" });
    }
  }, [showCollaboration, collaboratorCode]);
  ...
  const resetCode = useCallback(() => {
    ...
    toast.info("Code editor cleared!", { icon: "🧹" });
  }, []);
  ...
  const copyToClipboard = useCallback(() => {
    navigator.clipboard.writeText(code)
      ...
      .catch((err) => {
        toast.error("Failed to copy code", { icon: "❌" });
  ...
  const speakOutput = useCallback(() => {
    if ('speechSynthesis' in window) {
      ...
      toast.error("Text-to-speech is not supported in your browser.", { icon: "🔇" });
    }
  }, [output]);

  const addToHistory = useCallback((codeSnap) => {
    if (codeSnap.trim()) {
      const newHistory = [
        {
          code: codeSnap,
      ...
      toast.success("Code saved to history!", { icon: "💾" });
    }
  }, [codeHistory, language, currentQuestion]);

  const loadFromHistory = useCallback((index) => {
    setCode(codeHistory[index].code);
    setLanguage(codeHistory[index].language);
    toast.info("Code loaded from history", { icon: "⏳" });
  }, [codeHistory]);

  const showSolution = useCallback(() => {
    if (!hasCompiled) {
      toast.warning("Please compile your code first to see the solution!", { icon: "🔒" });
      return;
    }
    
    setCode(currentQuestion.solution);
    toast.info("Solution loaded. Study and understand it!", { icon: "🔓" });
  }, [hasCompiled, currentQuestion]);

  const requestAiHelp = useCallback(async () => {
    if (!aiPrompt.trim()) {
      toast.warning("Please enter your question for the AI assistant", { icon: "❓" });
      return;
    }

    try {
      setIsLoading(true);
      toast.info("AI assistant is analyzing your code...", { icon: <FaRobot /> });
      ...
      setAiResponse(response);
      toast.success("AI assistant has provided guidance!", { icon: "🤖" });
    } catch (error) {
      toast.error("AI assistant failed to respond", { icon: "❌" });
    } finally {
      setIsLoading(false);
    }
  }, [aiPrompt, difficulty, currentQuestion, language]);
  ...
  const uploadCode = useCallback((e) => {
    ...
    reader.onload = (event) => {
      ...
      toast.success("Code loaded from file!", { icon: "📂" });
    };
  ...
  const toggleRecording = useCallback(() => {
    ...
    toast.info(`Code recording ${!isRecording ? "started" : "stopped"}`, { icon: "🎥" });
  }, [isRecording]);

  const increaseFontSize = useCallback(() => {
    setFontSize(prev => Math.min(prev + 1, 24));
    toast.info(`Font size increased to ${fontSize + 1}px`, { icon: "🔍" });
  }, [fontSize]);

  const decreaseFontSize = useCallback(() => {
    setFontSize(prev => Math.max(prev - 1, 10));
    toast.info(`Font size decreased to ${fontSize - 1}px`, { icon: "🔎" });
  }, [fontSize]);
              ...
              <div className="card-body">
                ...
                <div className="mt-3 d-flex flex-wrap gap-2">
                  ...
                  <motion.button 
                    ...
                    onClick={() => {
                      toast.info(currentQuestion.explanation);
                    }}
                  >
